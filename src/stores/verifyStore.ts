...
type State = {
  ...
  unknownScans: Map<string, number>;
  orderCatalog: Map<string, number>;
};
...
export function useVerifyStore() {
  ...
  const totals = computed(() => {
    ...
    rows.value.forEach(r => (r.status === 'OK' ? ok++ : r.status === 'PARTIAL' ? partial++ : bad++));
    return { ok, partial, bad, total: rows.value.length };
  });

  function resetScans() { state.scanned.clear(); state.unknownScans.clear(); }
  function resetAll() { state.verifyCatalog.clear(); resetScans(); state.orderCatalog.clear(); }

  function loadVerifyCatalog(items: VerifyCatalogRow[]) {
    state.verifyCatalog.clear();
    for (const { barcode, qty } of items) {
      const k = String(barcode).trim(); if (!k) continue;
  ...
  function recordScan(barcode: string, qty = 1): Status {
    const k = String(barcode).trim(); if (!k) return 'BAD';
  ...
  function exportVerifyToCSV(filename = 'verify.csv') { exportVerifyCSV(filename, rows.value); }
  async function exportVerifyToPDF(filename = 'verify.pdf') { await exportVerifyPDF(filename, rows.value); }

  return { state, rows, totals, loadVerifyCatalog, loadOrderCatalog, recordScan, resetScans, resetAll, exportVerifyToCSV, exportVerifyToPDF };
}