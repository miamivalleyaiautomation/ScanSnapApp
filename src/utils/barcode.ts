/* Supported formats (BarcodeDetector / vue-qrcode-reader) */
export type Format =
  | 'aztec' | 'code_128' | 'code_39' | 'code_93' | 'codabar'
  | 'databar' | 'databar_limited' | 'databar_expanded'
  | 'data_matrix' | 'dx_film_edge' | 'ean_13' | 'ean_8'
  | 'itf' | 'maxi_code' | 'micro_qr_code' | 'pdf417'
  | 'qr_code' | 'rm_qr_code' | 'upc_a' | 'upc_e'
...
/* Defaults: conservative; EAN/UPC keep 1-digit trim visible option */
export const DEFAULT_TRIMS: TrimRules = Object.fromEntries(
  ALL_FORMATS.map(f => [f, { prefix: 0, suffix: 0 }])
) as TrimRules
...
/* Groups for quick toggles in Setup */
export const LINEAR_GROUP: Format[] = [
  'code_128','code_39','code_93','codabar','databar','databar_limited','databar_expanded','ean_13','ean_8','itf','upc_a','upc_e','dx_film_edge'
]
export const MATRIX_GROUP: Format[] = [
  'qr_code','micro_qr_code','rm_qr_code','data_matrix','pdf417','aztec','maxi_code'
]

/* Check digit helpers for EAN/UPC */
...
export function validateCheckDigit(code: string, fmt: Format): boolean {
  ...
  /* Skip strict validation for upc_e and others */
  return true
}
